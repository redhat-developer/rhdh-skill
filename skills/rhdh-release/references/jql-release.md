...
- **Placeholders:** `{{RELEASE_VERSION}}`
- **Notes:** All open work — used for Code Freeze announcements. Each issue type branch is parenthesized to ensure `project` and `fixVersion` apply to all branches.
...
- **Placeholders:** `{{RELEASE_VERSION}}`, `{{CLOUD_ID}}`
- **Notes:** Same as `code_freeze_issues` but scoped to a single team by Cloud ID. The `"Team[Team]"` clause applies after the outer parentheses, ensuring it filters all issue type branches.
