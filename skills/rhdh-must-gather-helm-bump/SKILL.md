...
- [ ] Distgit mirror matches upstream helm-related files; `vendor/helm` absent on `mode=cgw`
- [ ] Tekton `prefetch-input` matches install path (`generic` vs `gomod`) for must-gather only
...
- [konflux-tekton-updates](../konflux-tekton-updates/SKILL.md) — task-bundle digest bumps in the same midstream `.tekton` trees
