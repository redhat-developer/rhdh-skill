# RHDH Plugin Development Patterns
...
Use the target RHDH and Backstage versions established in `SKILL.md`. Your
`@backstage/*` dependency versions must match the target RHDH version.
Mismatches cause runtime errors — most commonly "Cannot read properties of
undefined." To move those versions, invoke `/rhdh-backstage-upgrade` by name.
...
plugins not yet migrated. To migrate one, invoke `/rhdh-plugin-nfs-migration`
by name.
...
public exports. List the changed reports among the files you leave behind; CI
checks them. All public exports need `/** @public */` JSDoc with a description
(not just the bare tag).

Changesets, staging, and signed-off commits belong to `/rhdh-pr-create`. Hand
it a concise release note and the verification evidence; do not create
publication state here.
