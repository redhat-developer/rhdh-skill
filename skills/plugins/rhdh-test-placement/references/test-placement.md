...
Paths are prefixed by repository: `rhdh:`, `overlays:`
(rhdh-plugin-export-overlays), and `plugins:` (rhdh-plugins). Discover current
repository layout from the target checkout. Some harnesses below may still be
in review; verify every referenced path on the target repo's current default
branch before recommending it.
