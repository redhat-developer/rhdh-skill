...
3. **Deploy locally** — Invoke `/rhdh-local` by name to deploy and test. Give
   it the exact package reference, the plugin configuration, the names of any
   required environment variables, and the verification checks below. Consume
   the evidence it reports rather than opening its files.
